...
now uses live backend streaming for one-shot runs, cross-image execution, and
the already running same-image case, so command output is no longer buffered
before being emitted as stream events.
