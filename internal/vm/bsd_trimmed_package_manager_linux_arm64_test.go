...
func TestRuntimeBootsTrimmedBSDSourceCachePackageManagers(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			inst, err := backend.startBSDArm64ManagedStream(ctx, client.CreateInstanceRequest{
				ID:       "trimmed-" + tc.def.BootKind,
				Image:    tc.def.Profile.Canonical,
				MemoryMB: tc.memoryMB,
			}, nil, tc.def, startTrimmedBSDArm64Session(tc.def.BootKind))
			if err != nil {
...
func startTrimmedBSDArm64Session(kind string) func(context.Context, kvm.OpenBSDManagedConfig, func(client.BootEvent) error) (*kvm.ManagedSession, error) {
	switch kind {
	case "freebsd":
		return func(ctx context.Context, cfg kvm.OpenBSDManagedConfig, onEvent func(client.BootEvent) error) (*kvm.ManagedSession, error) {
	...
	default:
		return kvm.StartOpenBSDManagedSession
	}
}
