...
type managedMountState struct {
	mu          sync.Mutex
	shares      map[string]client.ShareMount
	imageMounts map[string]string
}

func (s *managedMountState) AddShare(rootFS virtio.ShareMounter, share client.ShareMount, unsupportedFeature string, build func(client.ShareMount) (virtio.ShareMount, error)) error {
	if s == nil {
...
func (s *managedMountState) AddImage(rootFS virtio.ShareMounter, mountPath string, image *oci.Image, backend virtio.FSBackend) error {
	if s == nil {
...
func mountAlternateImageWithShares(ctx context.Context, inst Instance, mounter alternateImageMounter, mountPath string, image *oci.Image, shares []client.ShareMount) error {
	if mounter == nil {
