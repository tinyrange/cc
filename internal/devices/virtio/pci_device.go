...
// 	devNum         uint8
// 	funcNum        uint8
// 	pciHost        *pcipkg.HostBridge
// 	endpointHandle *pcipkg.DeviceHandle
...
// func newPCIDevice(vm hv.VirtualMachine, host *pcipkg.HostBridge, busNum, devNum, funcNum uint8, virtioDeviceID, subsystemDeviceID uint16, featureBits []uint64, handler deviceHandler) (*pciDevice, error) {
// 	if handler == nil {
...
// func (d *pciDevice) readMMIO(addr uint64, data []byte) error {
// 	return d.mmioAccess(addr, data, false)
// }

// func (d *pciDevice) writeMMIO(addr uint64, data []byte) error {
// 	return d.mmioAccess(addr, data, true)
// }

// func (d *pciDevice) mmioAccess(addr uint64, data []byte, write bool) error {
...
// func (d *pciDevice) readMSIXTable(addr uint64, data []byte) error {
// 	if len(data) == 0 {
// 		return nil
// 	}
...
// func (d *pciDevice) writeMSIXTable(addr uint64, data []byte) error {
// 	if len(data) == 0 {
// 		return nil
// 	}
// 	if len(d.msixEntries) == 0 {
...
// func (d *pciDevice) readMSIXPBA(addr uint64, data []byte) error {
// 	if len(data) == 0 {
// 		return nil
// 	}
...
// func (d *pciDevice) msixEntryByte(entryIdx, entryOffset int) byte {
...
// func (d *pciDevice) writeMSIXEntryByte(entryIdx, entryOffset int, value byte) {
// 	entry := &d.msixEntries[entryIdx]
// 	switch {
// 	case entryOffset < 8:
// 		shift := uint(entryOffset * 8)
...
// 		if virtioPCIDebug {
// 			slog.Info("virtio-pci: msix entry update", "entry", entryIdx, "addr", fmt.Sprintf("0x%x", entry.addr), "data", fmt.Sprintf("0x%x", entry.data), "masked", entry.masked)
// 		}
...
// func (d *pciDevice) writeCommonBlock(offset uint32, data []byte) error {
// 	for len(data) > 0 {
...
// func (d *pciDevice) readCommonBlock(offset uint32, data []byte) error {
// 	for len(data) > 0 {
// 		width := commonFieldWidth(offset)
// 		if width == 0 || len(data) < int(width) {
...
// func (d *pciDevice) setMemoryBAR(index int, size uint64) {
...
// func (d *pciDevice) setMemoryBAR64(index int, size uint64) {
// 	if index < 0 || index >= len(d.bars) {
// 		return
// 	}
// 	attrs := uint32(0x4)
...
// func (d *pciDevice) baseBAR(index int) *pciBAR {
// 	if index < 0 || index >= len(d.bars) {
// 		return nil
// 	}
...
// func (d *pciDevice) barIsHigh(index int) bool {
// 	if index < 0 || index >= len(d.bars) {
// 		return false
// 	}
...
// func (d *pciDevice) barBase(index uint8) uint64 {
...
// func (d *pciDevice) recomputeRegionAddrs() {
...
// func (d *pciDevice) mmioRegions() []hv.MMIORegion {
...
// func (d *pciDevice) configureMSICapability(offset uint16, next uint8) {
// 	if !d.exposeMSI {
// 		return
// 	}
...
// func (d *pciDevice) configureMSIXCapability(offset uint16) {
// 	if !d.exposeMSIX {
// 		return
// 	}
...
// func (d *pciDevice) msixVectorCount() int {
...
// func (d *pciDevice) configureVirtioCapabilities(start uint16) {
// 	if start == 0 {
// 		return
// 	}
...
// func (d *pciDevice) updateCapabilityChain() {
...
// func (d *pciDevice) updateMSIXControl(value uint16) {
// 	if !d.supportsMSIX {
// 		return
// 	}
// 	sizeBits := uint16(0)
...
// 	if virtioPCIDebug {
// 		slog.Info("virtio-pci: msix control update", "control", fmt.Sprintf("0x%04x", d.msixControl))
// 	}
...
// func (d *pciDevice) allocateBARs() error {
...
// func (d *pciDevice) allocateBAR(index int) error {
// 	if index < 0 || index >= len(d.bars) {
...
// func (d *pciDevice) initVirtioCap(buf []byte, next uint8, cfgType uint8, bar uint8, offset uint32, length uint32) {
...
// func (d *pciDevice) registerCapability(offset uint16) {
...
// func (d *pciDevice) readCommon(offset uint32, width uint32) (uint32, error) {
// 	switch width {
// 	case 1, 2:
...
// func (d *pciDevice) writeCommon(offset uint32, value uint32, width uint32) error {
// 	switch width {
// 	case 1, 2:
// 		mask := uint32((1 << (width * 8)) - 1)
...
// func (d *pciDevice) readDeviceConfig(offset uint32, width uint32) (uint32, error) {
...
// func (d *pciDevice) writeDeviceConfig(offset uint32, value uint32, width uint32) error {
// 	aligned := offset &^ 0x3
...
// func (d *pciDevice) readMSICap(offset uint16) (uint32, bool) {
// 	if d.msiCapOffset == 0 {
// 		return 0, false
// 	}
// 	base := d.msiCapOffset
...
// 	case base + 8:
// 		if d.msiControl&msiControl64BitCap != 0 { // 64-bit capable
...
// func (d *pciDevice) writeMSICap(offset uint16, value uint32) bool {
// 	if d.msiCapOffset == 0 {
// 		return false
// 	}
// 	base := d.msiCapOffset
...
// 		if d.msiControl&0x1 != 0 {
// 			slog.Info("virtio-pci: MSI enabled")
// 		}
// 		return true
// 	case base + 4:
// 		d.msiAddress = (d.msiAddress & 0xffff_ffff00000000) | uint64(value)
// 		slog.Info("virtio-pci: MSI address", "addr", fmt.Sprintf("0x%x", d.msiAddress))
// 		return true
// 	case base + 8:
// 		if d.msiControl&msiControl64BitCap != 0 {
...
// 		}
// 		if d.msiControl&msiControl64BitCap == 0 {
// 			slog.Info("virtio-pci: MSI data", "data", d.msiData)
// 		}
// 		return true
// 	case base + 12:
// 		if d.msiControl&msiControl64BitCap != 0 {
...
// func (d *pciDevice) readMSIXCap(offset uint16) (uint32, bool) {
// 	if d.msixCapOffset == 0 {
// 		return 0, false
// 	}
...
// func (d *pciDevice) writeMSIXCap(offset uint16, value uint32) bool {
// 	if d.msixCapOffset == 0 {
// 		return false
// 	}
...
// 	}
// 	if virtioPCIDebug {
// 		slog.Info("virtio-pci: msix cap write", "value", fmt.Sprintf("0x%08x", value))
// 	}
// 	d.updateMSIXControl(uint16(value >> 16))
// 	return true
// }

// func (d *pciDevice) readVirtioCap(offset uint16) (uint32, bool) {
...
// func (d *pciDevice) ConfigSpace() pcipkg.ConfigSpace {
// 	return d
// }

// func (d *pciDevice) ReadConfig(offset uint16, size uint8) (uint32, error) {
...
// func (d *pciDevice) WriteConfig(offset uint16, size uint8, value uint32) error {
// 	if size != 1 && size != 2 && size != 4 {
...
// func (d *pciDevice) readConfigDWord(offset uint16) (uint32, error) {
// 	switch offset {
// 	case 0x00:
...
// func (d *pciDevice) writeConfigDWord(offset uint16, value uint32) error {
// 	switch offset {
// 	case 0x04:
...
// func (d *pciDevice) readBAR(offset uint16) (uint32, error) {
...
// func (d *pciDevice) writeBAR(offset uint16, value uint32) error {
// 	if offset < type0BAROffset {
...
// func (d *pciDevice) OnBARReprogram(index int, value uint32) error {
// 	if index < 0 || index >= len(d.bars) {
// 		return fmt.Errorf("BAR index %d out of range", index)
// 	}
...
// func (d *pciDevice) handlePCIConfigRead(offset uint32, size uint8) uint32 {
// 	val, err := d.ReadConfig(uint16(offset), size)
// 	if err != nil {
// 		return 0
// 	}
// 	return val
...
// func (d *pciDevice) handlePCIConfigWrite(offset uint32, value uint32, size uint8) {
// 	_ = d.WriteConfig(uint16(offset), size, value)
// }

// func (d *pciDevice) handleCommonCfgRead(offset uint32) (uint32, error) {
// 	switch offset {
// 	case VIRTIO_PCI_COMMON_DFSELECT:
...
// func (d *pciDevice) handleCommonCfgWrite(offset uint32, value uint32) error {
// 	switch offset {
// 	case VIRTIO_PCI_COMMON_DFSELECT:
...
// 		if virtioPCIDebug {
// 			slog.Info("virtio-pci: queue select", "queue", d.queueSel)
// 		}
// 	case VIRTIO_PCI_COMMON_Q_SIZE:
// 		if q := d.currentQueue(); q != nil {
// 			if value == 0 {
...
// 			q.size = uint16(value)
// 			if virtioPCIDebug {
// 				slog.Info("virtio-pci: queue size", "queue", d.queueSel, "size", value)
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_MSIX:
// 		if d.supportsMSIX {
...
// 				if virtioPCIDebug {
// 					slog.Info("virtio-pci: queue msix vector", "queue", d.queueSel, "vector", q.msixVector)
// 				}
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_ENABLE:
...
// 				q.enable = false
// 				if virtioPCIDebug {
// 					slog.Info("virtio-pci: queue disabled", "queue", d.queueSel)
// 				}
// 			} else {
// 				if q.size == 0 {
...
// 				// 	return fmt.Errorf("queue enable set before queue addresses")
// 				// }
// 				q.ready = true
// 				q.enable = true
// 				if virtioPCIDebug {
// 					slog.Info("virtio-pci: queue enabled", "queue", d.queueSel, "size", q.size, "desc", fmt.Sprintf("0x%x", q.descAddr), "avail", fmt.Sprintf("0x%x", q.availAddr), "used", fmt.Sprintf("0x%x", q.usedAddr))
// 				}
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_DESCLO:
...
// 			if virtioPCIDebug {
// 				slog.Info("virtio-pci: queue desc lo", "queue", d.queueSel, "addr", fmt.Sprintf("0x%x", q.descAddr))
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_DESCHI:
// 		if q := d.currentQueue(); q != nil {
...
// 			if virtioPCIDebug {
// 				slog.Info("virtio-pci: queue desc hi", "queue", d.queueSel, "addr", fmt.Sprintf("0x%x", q.descAddr))
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_AVAILLO:
// 		if q := d.currentQueue(); q != nil {
...
// 			if virtioPCIDebug {
// 				slog.Info("virtio-pci: queue avail lo", "queue", d.queueSel, "addr", fmt.Sprintf("0x%x", q.availAddr))
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_AVAILHI:
// 		if q := d.currentQueue(); q != nil {
...
// 			if virtioPCIDebug {
// 				slog.Info("virtio-pci: queue avail hi", "queue", d.queueSel, "addr", fmt.Sprintf("0x%x", q.availAddr))
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_USEDLO:
// 		if q := d.currentQueue(); q != nil {
...
// 			if virtioPCIDebug {
// 				slog.Info("virtio-pci: queue used lo", "queue", d.queueSel, "addr", fmt.Sprintf("0x%x", q.usedAddr))
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_USEDHI:
// 		if q := d.currentQueue(); q != nil {
...
// 			if virtioPCIDebug {
// 				slog.Info("virtio-pci: queue used hi", "queue", d.queueSel, "addr", fmt.Sprintf("0x%x", q.usedAddr))
// 			}
// 		}
// 	case VIRTIO_PCI_COMMON_Q_NOFF:
// 		// read-only
...
// func (d *pciDevice) handleNotifyWrite(offset uint32, value uint16) error {
// 	queueIdx := int(value)
...
// func (d *pciDevice) handleISRRead() uint8 {
// 	value := d.interruptStatus
// 	d.interruptStatus = 0
// 	return value
// }

// func (d *pciDevice) handleDeviceCfgRead(offset uint32) (uint32, error) {
// 	if d.handler != nil {
...
// func (d *pciDevice) handleDeviceCfgWrite(offset uint32, value uint32) error {
// 	if d.handler != nil {
...
// func (d *pciDevice) currentQueue() *queue {
// 	idx := int(d.queueSel)
...
// func (d *pciDevice) queue(index int) *queue {
...
// func (d *pciDevice) raiseInterrupt(queueIndex int, bit uint32) {
// 	d.interruptStatus |= uint8(bit)
// 	if d.vm == nil {
// 		return
// 	}
// 	if d.msixEnabled() {
// 		vector := d.msixVectorFor(queueIndex)
...
// 				slog.Error("virtio-pci: signal MSI failed", "bus", d.busNum, "device", d.devNum, "function", d.funcNum, "err", err)
// 			}
// 			return
// 		}
// 	}
...
// 		return
// 	}
// 	if d.command&pciCommandIntxDisable != 0 {
// 		return
// 	}
// 	if vm, ok := d.vm.(irqPulsar); ok {
// 		if err := vm.PulseIRQ(d.irqLine); err != nil {
// 			slog.Error("virtio-pci: pulse irq failed", "irq", d.irqLine, "bus", d.busNum, "device", d.devNum, "function", d.funcNum, "err", err)
// 		}
// 	}
// }

// func (d *pciDevice) msixEnabled() bool {
// 	return d.supportsMSIX && (d.msixControl&msixControlEnableBit) != 0
// }

// func (d *pciDevice) msixVectorFor(queueIndex int) uint16 {
// 	if queueIndex >= 0 {
...
// 	}
// 	return d.msixConfigVector
// }

// func (d *pciDevice) trySignalMSIX(vector uint16) (bool, bool) {
// 	if !d.msixEnabled() {
// 		return false, false
// 	}
...
// 		slog.Error("virtio-pci: signal MSI-X failed", "bus", d.busNum, "device", d.devNum, "function", d.funcNum, "vector", vector, "err", err)
// 		return false, false
// 	}
...
// func (d *pciDevice) setMSIXPendingBit(vector uint16) {
// 	if int(vector) >= len(d.msixEntries) {
// 		return
// 	}
// 	idx := int(vector) / 64
...
// func (d *pciDevice) clearMSIXPendingBit(vector uint16) {
// 	if int(vector) >= len(d.msixEntries) {
// 		return
// 	}
// 	idx := int(vector) / 64
...
// func (d *pciDevice) emitPendingVector(vector uint16) {
// 	idx := int(vector) / 64
// 	if idx < 0 || idx >= len(d.msixPending) {
// 		return
// 	}
...
// func (d *pciDevice) flushMSIXPending() {
...
// func (d *pciDevice) msiEnabled() bool {
// 	if d.msixEnabled() {
// 		return false
// 	}
...
// func (d *pciDevice) readAvailState(q *queue) (uint16, uint16, error) {
...
// func (d *pciDevice) readAvailEntry(q *queue, ringIndex uint16) (uint16, error) {
// 	if err := ensureQueueReady(q); err != nil {
// 		return 0, err
// 	}
// 	var entry [2]byte
// 	offset := q.availAddr + 4 + uint64(ringIndex%q.size)*2
// 	if err := d.readGuestInto(offset, entry[:]); err != nil {
// 		return 0, err
// 	}
// 	return binary.LittleEndian.Uint16(entry[:]), nil
// }

// func (d *pciDevice) readDescriptor(q *queue, index uint16) (virtqDescriptor, error) {
// 	if err := ensureQueueReady(q); err != nil {
// 		return virtqDescriptor{}, err
// 	}
// 	var desc virtqDescriptor
// 	buf := make([]byte, 16)
// 	offset := q.descAddr + uint64(index%q.size)*16
// 	if err := d.readGuestInto(offset, buf); err != nil {
// 		return virtqDescriptor{}, err
// 	}
// 	desc.addr = binary.LittleEndian.Uint64(buf[0:8])
// 	desc.length = binary.LittleEndian.Uint32(buf[8:12])
// 	desc.flags = binary.LittleEndian.Uint16(buf[12:14])
// 	desc.next = binary.LittleEndian.Uint16(buf[14:16])
// 	return desc, nil
// }

// func (d *pciDevice) recordUsedElement(q *queue, head uint16, length uint32) error {
// 	if err := ensureQueueReady(q); err != nil {
// 		return err
// 	}
...
// func (d *pciDevice) guestFeatureEnabled(bit uint32) bool {
// 	index := bit / 32
// 	offset := bit % 32
...
// func (d *pciDevice) eventIdxEnabled() bool {
// 	return d.guestFeatureEnabled(virtioRingFeatureEventIdxBit)
// }

// func (d *pciDevice) setAvailEvent(q *queue, value uint16) error {
// 	if err := ensureQueueReady(q); err != nil {
// 		return err
// 	}
...
// func (d *pciDevice) readGuest(addr uint64, length uint32) ([]byte, error) {
...
// func (d *pciDevice) writeGuest(addr uint64, data []byte) error {
// 	if len(data) == 0 {
// 		return nil
// 	}
...
// func (d *pciDevice) readGuestInto(addr uint64, buf []byte) error {
// 	if len(buf) == 0 {
// 		return nil
// 	}
// 	if d.vm == nil {
...
// func (d *pciDevice) writeGuestFrom(addr uint64, data []byte) error {
// 	if len(data) == 0 {
// 		return nil
// 	}
// 	if d.vm == nil {
...
// func (d *pciDevice) writeGuestUint16(addr uint64, value uint16) error {
// 	var buf [2]byte
// 	binary.LittleEndian.PutUint16(buf[:], value)
// 	return d.writeGuestFrom(addr, buf[:])
// }

// func (d *pciDevice) writeGuestUint32(addr uint64, value uint32) error {
// 	var buf [4]byte
