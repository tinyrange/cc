...
func main() {
	if err := run(); err != nil {
		writeConsole("ccx3-openbsd-init: " + err.Error() + "\n")
		for {
...
func commandLoop(control net.Conn) error {
	...
	for {
		...
		if err := json.Unmarshal(line, &req); err != nil {
			writeConsole("ccx3-openbsd-init: decode request: " + err.Error() + "\n")
			continue
		}
		if req.Kind == "" {
			req.Kind = "exec"
...
func writeErr(control io.Writer, id string, err error) {
	writeBytes(control, errMarker, id, []byte("ccx3-openbsd-init: "+err.Error()+"\n"))
}
