...
func TestNetworkHTTPDownload64KiB(t *testing.T) {
	if err := configureInterfaceIP("eth0", net.ParseIP("10.42.0.2"), net.CIDRMask(24, 32)); err != nil {
		t.Fatalf("failed to configure IP address: %v", err)
	}

	client := &http.Client{Timeout: 2 * time.Second}
	resp, err := client.Get(fmt.Sprintf("http://10.42.0.1:4244/download/%d", 64*1024))
	if err != nil {
...
func TestNetworkHTTPDownload64KiBAgain(t *testing.T) {
	if err := configureInterfaceIP("eth0", net.ParseIP("10.42.0.2"), net.CIDRMask(24, 32)); err != nil {
		t.Fatalf("failed to configure IP address: %v", err)
	}
...
func TestNetworkHTTPDownload100KiB(t *testing.T) {
	if err := configureInterfaceIP("eth0", net.ParseIP("10.42.0.2"), net.CIDRMask(24, 32)); err != nil {
		t.Fatalf("failed to configure IP address: %v", err)
	}

	client := &http.Client{Timeout: 2 * time.Second}
	resp, err := client.Get(fmt.Sprintf("http://10.42.0.1:4244/download/%d", 100*1024))
	if err != nil {
		t.Fatalf("http get: %v", err)
	}
	defer resp.Body.Close()
