...
func TestRasterizerWindingReachesHostFramebufferUnchanged(t *testing.T) {
	host, err := newDarwinHost()
	if err != nil {
		t.Fatal(err)
	}
...
void main() {
	vec2 position = vec2(float((gl_VertexID << 1) & 2), float(gl_VertexID & 2));
	gl_Position = vec4(position.yx * 2.0 - 1.0, 0.0, 1.0);
}`
...
func TestDrawDisablesRetiredVertexAttributes(t *testing.T) {
	...
	context.surfaces[11] = hostSurface{resourceID: color.ID}
	...
	context.vertexBuffers[0] = hostVertexBuffer{stride: 8, resourceID: positions.ID}
	context.vertexBuffers[1] = hostVertexBuffer{stride: 8, resourceID: coordinates.ID}
...
void main() { color = vec4(1.0, varyingCoordinate.x * 0.0, 0.0, 1.0); }`}
	...
	if err := host.dispatch(func() error {
		...
		host.enabledVertexAttributes = 3
...
func TestBufferTransfersHonorInlineAndBackingOffsets(t *testing.T) {
	...
	if err := host.dispatch(func() error {
		host.gl.bindBuffer(glArrayBuffer, host.resources[description.ID].buffer)
