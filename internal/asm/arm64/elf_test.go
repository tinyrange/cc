...
func TestStandaloneELFHeader(t *testing.T) {
	frag := asm.Group{
		SyscallWriteString(asm.Immediate(1), "Hello, ARM64 ELF!\n"),
		exit(0),
	}
	...
	if got, want := f.FileHeader.Machine, elf.EM_AARCH64; got != want {
		t.Fatalf("ELF machine=%v, want %v", got, want)
	}
	...
	if !bytes.Contains(elfBytes[cfg.SegmentOffset:], []byte("Hello, ARM64 ELF!\n")) {
		t.Fatalf("standalone ELF missing literal string")
	}
}

func TestStandaloneELFRelocations(t *testing.T) {
	msg := append([]byte("hi arm64"), 0)
	constVar := asm.Variable(100)
	frag := asm.Group{
		LoadConstantBytes(constVar, msg),
		SyscallWrite(asm.Immediate(1), asm.Register(constVar), asm.Immediate(len(msg)-1)),
		exit(0),
	}
	prog, err := EmitProgram(frag)
	if err != nil {
