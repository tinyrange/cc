...
Redirect output to writers. In Go, output streams to the writer in real time as the command produces it. Other languages use pipes for the same effect.
...
```
  ...
  <LangTabItem lang="python">
```python
# Use combined_output() for buffered capture
output = inst.command("sh", "-c", "echo out; echo err >&2").combined_output()

# Or use pipes for streaming (see Pipes section below)
```
  </LangTabItem>
  <LangTabItem lang="rust">
```rust
...
```
  ...
  <LangTabItem lang="typescript">
```typescript
// Use combinedOutput() for buffered capture
...
```
  ...
  <LangTabItem lang="c">
```c
// Use cc_cmd_combined_output() for buffered capture
// Or use pipes for streaming (see Pipes section below)
```
  </LangTabItem>
</LanguageTabs>
...
Get pipes for streaming I/O. Pipes provide fine-grained control over command input and output, allowing you to read stdout/stderr incrementally and write to stdin while the command is running. Pipe methods must be called before `Start()`.
...
let mut stdin = cmd.stdin_pipe()?;
let mut stdout = cmd.stdout_pipe()?;
cmd.start()?;
...
const stdin = await cmd.stdinPipe();
...
cc_conn stdin_pipe, stdout_pipe;
cc_cmd_stdin_pipe(cmd, &stdin_pipe, &err);
cc_cmd_stdout_pipe(cmd, &stdout_pipe, &err);
cc_cmd_start(cmd, &err);

// Write to stdin and close to signal EOF
cc_conn_write(stdin_pipe, (const uint8_t*)"hello", 5, &written, &err);
cc_conn_close(stdin_pipe, &err);

// Read from stdout
uint8_t buf[256];
size_t n;
cc_conn_read(stdout_pipe, buf, sizeof(buf), &n, &err);
...
<LanguageTabs>
  <LangTabItem lang="go">
```go
cmd := instance.Command("sh", "-c", "echo err >&2")
...
```
  ...
  <LangTabItem lang="rust">
```rust
let mut cmd = inst.command("sh", &["-c", "echo err >&2"])?;
let mut stderr = cmd.stderr_pipe()?;
cmd.start()?;
let mut buf = [0u8; 256];
...
```
  ...
  <LangTabItem lang="typescript">
```typescript
const cmd = await inst.command('sh', '-c', 'echo err >&2');
...
```
  ...
  <LangTabItem lang="c">
```c
const char* args[] = {"-c", "echo err >&2", NULL};
cc_cmd cmd;
cc_cmd_new(inst, "sh", args, &cmd, &err);
cc_conn stderr_pipe;
cc_cmd_stderr_pipe(cmd, &stderr_pipe, &err);
cc_cmd_start(cmd, &err);
uint8_t buf[256];
size_t n;
cc_conn_read(stderr_pipe, buf, sizeof(buf), &n, &err);
cc_conn_close(stderr_pipe, &err);
int exit_code;
cc_cmd_wait(cmd, &exit_code, &err);
```
  </LangTabItem>
