...
```
  ...
  <LangTabItem lang="python">
```python
options = cc.InstanceOptions(cpus=2)
inst = cc.Instance(source, options)
```
  </LangTabItem>
  <LangTabItem lang="rust">
```rust
let opts = InstanceOptions { cpus: 2, ..Default::default() };
let inst = Instance::new(source, Some(opts))?;
```
  </LangTabItem>
  <LangTabItem lang="typescript">
```typescript
const inst = await source.createInstance({ cpus: 2 });
```
  </LangTabItem>
  <LangTabItem lang="c">
```c
cc_instance_options opts = { .cpus = 2 };
cc_instance_new(source, &opts, &inst, &err);
```
  </LangTabItem>
</LanguageTabs>
