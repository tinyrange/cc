...
class PyNeurodeskClient:
    ...
    def ensure_instance(
        ...
        *,
        ...
        if state.status == "running" and state.image not in ("", None, image):
            ...
            return self.create_instance(image, timeout=timeout, dmesg=dmesg)
    ...
    def run_command(self, request: RunCommandRequest) -> CommandResult:
        response = self._client.post("/vm/run", json=request.to_payload())
        payload = self._decode_json(response)
